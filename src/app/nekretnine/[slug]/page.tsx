...
export function generateStaticParams() { return sveNekretnine().map(({ slug }) => ({ slug })) }

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params
  const item = sveNekretnine().find((i) => i.slug === slug)
